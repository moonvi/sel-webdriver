...
namespace Litecart
{
    ...
    public class MainPageTests : TestBase
    {
        ...
        public void CheckAddRemoveProductsFromBasket()
        {
            ...
            int cartProductsCount = homePage.GetCartQuantity();

            Assert.AreEqual(0, cartProductsCount);

            ProductPage productDetailsPage = new ProductPage(driver);
            int i = 1;
            int cartProductsQuantity;
            do
            {
                homePage.OpenMostPopularProduct(1);

                productDetailsPage.AddToCartWithDefailtSettings(i);
                productDetailsPage.NavigateToTheMainPage();

                cartProductsQuantity = homePage.GetCartQuantity();

                //check that product is added to the cart and cart is updated
                Assert.AreEqual(i, cartProductsQuantity);

                i++;
            } while (cartProductsQuantity != 3);

            CartPage cartPage = homePage.NavigateToCartPage();

            cartPage.RemoveAllProducts();
            homePage.OpenMainPage();
            cartProductsCount = homePage.GetCartQuantity();
            Assert.AreEqual(0, cartProductsCount);
        }
    }
}
